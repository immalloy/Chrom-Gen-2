...
all dependencies (including `numpy`, `wxPython`, and `parselmouth`) so the
resulting executable can run on Windows without a local Python installation.
...
2. From the repository root, run:
   ```
   pyinstaller chromatic_gen.spec --onefile
   ```
3. The bundled executable will be available in `dist/chromatic_gen.exe`.
